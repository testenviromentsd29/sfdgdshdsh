$(function () {
	window.addEventListener('message', function(event) {
		...
		if (action == 'decode') {
			let sid = event.data.sid;
			let text = event.data.text;

			let decoded = utf8_decode(text);

			$.post('https://customStatus/decode', JSON.stringify({sid: sid, text: decoded}));
			...
			$.post('https://customStatus/encode', JSON.stringify({identifier: identifier, status: status}));
		}
	});
});
