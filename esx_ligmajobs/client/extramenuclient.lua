...
Citizen.CreateThread(function()
	...
	while Job == nil or Job.Locations == nil do
		if IsControlJustPressed(1, Config.OpenExtraMenu) then
			ESX.ShowNotification("Loading Extra Menu")
		end
		Wait(0)
	end
	if not running and AmIMechanic() then
		running = true
		startIt()
	end
end)
...
AddEventHandler('esx:setJob', function(job)
	PlayerData.job = job
	running = false
	local prev = GetGameTimer()/1000
	while GetGameTimer()/1000 - prev < 30 do
		if IsControlJustPressed(1, Config.OpenExtraMenu) then
			ESX.ShowNotification("Loading Extra Menu")
		end
		Wait(0)
				...
				if AmIMechanic() then
					...
					if not menu then
						if IsPedInAnyVehicle(PlayerPedId(), 0) and (GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), 0), -1) == PlayerPedId()) then
							...
							local inVeh = GetVehiclePedIsIn(PlayerPedId())
							if inVeh ~= nil and inVeh ~= 0 then
								if GetVehicleEngineHealth(inVeh) >= 1000 then
									menu = true
								else
									drawNotification("~r~You need to have a fixed vehicle in order to open extra menu")
								end
							end
					...
					drawNotification("~g~You must be a Mechanic to open this!")
				end
			end
...
function AmIMechanic()
	if Job and Job.Locations then
		for k,v in pairs(Job.Locations) do
			if string.find(k,"MechanicalUpgrade") then
				return true
			end
		end
		return false
