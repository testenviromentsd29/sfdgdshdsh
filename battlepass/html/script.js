...
$(function() {
	...
	$('#btn-buy-level').click(function() {
		$('#wrap').hide();
		$('body').hide()
		$.post('https://battlepass/buy_level', JSON.stringify({}));
	});

	$('#btn-reset-level').click(function() {
		$('#wrap').hide();
		$('body').hide()
		$.post('https://battlepass/reset_battlepass', JSON.stringify({}));
	});
	...
	document.onkeyup = function(event) {
		if (event.key == 'Escape') {
			$('body').hide()
			$('#wrap').hide();
			$.post('https://battlepass/quit', JSON.stringify({}));
		}
	};
});
...
function ClaimReward() {
	$('body').hide()
	$('#wrap').hide();
	$.post('https://battlepass/reward', JSON.stringify({}));
}

function BuyLootbox(id, useCoin) {
	$('body').hide()
	$('#wrap').hide();
	$.post('https://battlepass/buy_lootbox', JSON.stringify({id, useCoin}));
}
	...
	<div id="exp-progress">
		...
		var bar = new ProgressBar.Circle(levelbar, {
		  ...
		  step: (state, bar) => {
			...
			bar.setText("<span id='level-circle'>`+ data.level +`</span><br/><br/><span id='level-circle-desc'>LEVEL</span>");
...
function GenerateRewards() {
	...
	for (let i = 0; i < data.rewards.length; i++) {
		...
		let itemReceived;
		let itemLevel = i + 1;
		let backgroundImg = `nui://esx_inventoryhud_matza/html/img/items/`+ data.rewards[i].item +`.png`
...
function GenerateLootboxes() {
	...
	for (let i = 0; i < data.lootboxes.length; i++) {
		...
		`<div id="lootbox-block">
			...
			<div id="lootbox-img" style="background:url('./images/lootbox/`+ data.lootboxes[i].name +`.png') center center no-repeat;background-size:cover;background-origin:content-box;"></div>
