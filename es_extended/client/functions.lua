...
CreateThread(function()
	...
	while true do
		if #ESX.UI.Menu.GetOpenedMenus() > 0 then
			if not vmenuDisabled then
				vmenuDisabled = true
				TriggerEvent("vMenu:enableMenu",false)
			end
		else
			if vmenuDisabled then
...
ESX.GetWeaponLabel = function(name)
    if WeaponLabels[name] then
        return WeaponLabels[name]
    else
        return false
    end
end

ESX.GetExperience = function(name)

	if ESX.PlayerData.Experiences and ESX.PlayerData.Experiences[name] then
		return ESX.PlayerData.Experiences[name].amount
	else
		if ESX.PlayerData.Experiences == nil then
			ESX.PlayerData.Experiences = {}
		end
		local toreturn
		ESX.TriggerServerCallback('esx:getmyxp',function(answer)
			ESX.PlayerData.Experiences[name] = answer
			toreturn = answer.amount
		end,name)
...
ESX.GetRank = function(experience_name)

	local xp = ESX.GetExperience(experience_name)
	if xp < 250 then
		return 1
	elseif xp >= 250 and xp < 500 then
		return 2
	elseif xp >= 500 and xp < 750 then
		return 3
	else
		return 4
	end
...
ESX.GetJobAttribute = function(job,key)
	if ESX.JobAttributes then
		if ESX.JobAttributes[job] then
			return ESX.JobAttributes[job][key]
		else
			return nil
		end
...
--[[ ESX.GetExperience = function(key)
	return ESX.PlayerData.attributes["xp_"..key]
end
 ]]
...
ESX.ShowNotification = function(msg)
	...
	--TriggerEvent('notify:showNotification', 'Notification', msg)
	exports['notify-sys']:Notify(msg)
end
...
ESX.Game.GetClosestEntity = function(entities, isPlayerEntities, coords, modelFilter)
	...
	if modelFilter then
		...
		if type(modelFilter) == "string" then
			local objstr = modelFilter
			modelFilter = {}
			modelFilter[GetHashKey(objstr)] = true
		end
	...
	if DoesEntityExist(vehicle) then
		...
		for k,v in pairs(data) do 
			...
			elseif k == "neonColor" then 
				if v[1] == 255 and v[2] == 0 and v[3] == 255 then 
					data[k] = nil 
				end
			...
			elseif k == "tyreSmokeColor" then 
				if v[1] == 255 and v[2] == 255 and v[3] == 255 then 
					data[k] = nil 
				end
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.windowTint then SetVehicleWindowTint(vehicle, props.windowTint or -1) end
		...
		if props.modSpoilers then SetVehicleMod(vehicle, 0, props.modSpoilers or -1, false) end
		if props.modFrontBumper then SetVehicleMod(vehicle, 1, props.modFrontBumper or -1, false) end
		if props.modRearBumper then SetVehicleMod(vehicle, 2, props.modRearBumper or -1, false) end
		if props.modSideSkirt then SetVehicleMod(vehicle, 3, props.modSideSkirt or -1, false) end
		if props.modExhaust then SetVehicleMod(vehicle, 4, props.modExhaust or -1, false) end
		if props.modFrame then SetVehicleMod(vehicle, 5, props.modFrame or -1, false) end
		if props.modGrille then SetVehicleMod(vehicle, 6, props.modGrille or -1, false) end
		if props.modHood then SetVehicleMod(vehicle, 7, props.modHood or -1, false) end
		if props.modFender then SetVehicleMod(vehicle, 8, props.modFender or -1, false) end
		if props.modRightFender then SetVehicleMod(vehicle, 9, props.modRightFender or -1, false) end
		if props.modRoof then SetVehicleMod(vehicle, 10, props.modRoof or -1, false) end
		if props.modEngine then SetVehicleMod(vehicle, 11, props.modEngine or -1, false) end
		if props.modBrakes then SetVehicleMod(vehicle, 12, props.modBrakes or -1, false) end
		if props.modTransmission then SetVehicleMod(vehicle, 13, props.modTransmission or -1, false) end
		if props.modHorns then SetVehicleMod(vehicle, 14, props.modHorns or -1, false) end
		if props.modSuspension then SetVehicleMod(vehicle, 15, props.modSuspension or -1, false) end
		if props.modArmor then SetVehicleMod(vehicle, 16, props.modArmor or -1, false) end
		if props.modTurbo ~= nil then 
			ToggleVehicleMod(vehicle,  18, props.modTurbo or 0) 
		end
		if props.modXenon then ToggleVehicleMod(vehicle,  22, props.modXenon or 255) end
		if props.modFrontWheels then SetVehicleMod(vehicle, 23, props.modFrontWheels or -1, false) end
		if props.modBackWheels then SetVehicleMod(vehicle, 24, props.modBackWheels or -1, false) end
		if props.modPlateHolder then SetVehicleMod(vehicle, 25, props.modPlateHolder or -1, false) end
		if props.modVanityPlate then SetVehicleMod(vehicle, 26, props.modVanityPlate or -1, false) end
		if props.modTrimA then SetVehicleMod(vehicle, 27, props.modTrimA or -1, false) end
		if props.modOrnaments then SetVehicleMod(vehicle, 28, props.modOrnaments or -1, false) end
		if props.modDashboard then SetVehicleMod(vehicle, 29, props.modDashboard or -1, false) end
		if props.modDial then SetVehicleMod(vehicle, 30, props.modDial or -1, false) end
		if props.modDoorSpeaker then SetVehicleMod(vehicle, 31, props.modDoorSpeaker or -1, false) end
		if props.modSeats then SetVehicleMod(vehicle, 32, props.modSeats or -1, false) end
		if props.modSteeringWheel then SetVehicleMod(vehicle, 33, props.modSteeringWheel or -1, false) end
		if props.modShifterLeavers then SetVehicleMod(vehicle, 34, props.modShifterLeavers or -1, false) end
		if props.modAPlate then SetVehicleMod(vehicle, 35, props.modAPlate or -1, false) end
		if props.modSpeakers then SetVehicleMod(vehicle, 36, props.modSpeakers or -1, false) end
		if props.modTrunk then SetVehicleMod(vehicle, 37, props.modTrunk or -1, false) end
		if props.modHydrolic then SetVehicleMod(vehicle, 38, props.modHydrolic or -1, false) end
		if props.modEngineBlock then SetVehicleMod(vehicle, 39, props.modEngineBlock or -1, false) end
		if props.modAirFilter then SetVehicleMod(vehicle, 40, props.modAirFilter or -1, false) end
		if props.modStruts then SetVehicleMod(vehicle, 41, props.modStruts or -1, false) end
		if props.modArchCover then SetVehicleMod(vehicle, 42, props.modArchCover or -1, false) end
		if props.modAerials then SetVehicleMod(vehicle, 43, props.modAerials or -1, false) end
		if props.modTrimB then SetVehicleMod(vehicle, 44, props.modTrimB or -1, false) end
		if props.modTank then SetVehicleMod(vehicle, 45, props.modTank or -1, false) end
		if props.modWindows then SetVehicleMod(vehicle, 46, props.modWindows or -1, false) end

		SetVehicleMod(vehicle, 48, props.modLivery or -1, false)
		SetVehicleLivery(vehicle, props.modLivery or -1)
...
AddEventHandler('esx:serverCallback', function(requestId, ...)
	...
	if ESX.ServerCallbacks[requestId] ~= nil then
		ESX.ServerCallbacks[requestId](...)
		ESX.ServerCallbacks[requestId] = nil
	else
		print('Callback with requestId: '..requestId..' does not exist')
	end
end)
...
ESX.GetPlayerJob = function(id)
	if ESX.OtherPlayerData == nil then
		return -1
	end
	if ESX.OtherPlayerData[id] then
		return ESX.OtherPlayerData[id].job
	else
		print("Job Of Id: "..id.." Not Found!! ["..tostring(GetInvokingResource()).."]")
		return nil
	end
end

ESX.GetPlayerJob2 = function(id)
	if ESX.OtherPlayerData == nil then
		return -1
	end
	if ESX.OtherPlayerData[id] then
		return ESX.OtherPlayerData[id].job2
	else
		print("Job2 Of Id: "..id.." Not Found!! ")
		return nil
	end
end

ESX.GetPlayerIdentifier = function(id)
	if ESX.OtherPlayerData == nil then
		return -1
	end
	if ESX.OtherPlayerData[id] then
		return ESX.OtherPlayerData[id].identifier
	else
		--print("Steam Identifier Of Id: "..id.." Not Found!! ")
		return "nil"
	end
end

ESX.GetPlayerId = function(id)
	if ESX.OtherPlayerData == nil then
		return -1
	end
	if ESX.OtherPlayerData[id] then
		return ESX.OtherPlayerData[id].id
	else
		print("DB ID Of Id: "..id.." Not Found!! ")
		return -1
	end
end