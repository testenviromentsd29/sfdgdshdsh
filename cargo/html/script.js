window.addEventListener('message', function(event) {
	if (event.data.action == "showTime") {
		...
		return;
	} else if (event.data.action == "showGuards") {
		$('#event-guards-wrap').show();
		this.document.getElementById('guards-left-text').innerHTML = event.data.guards
		return;
	} else if (event.data.action == "hideGuards") {
		$('#event-guards-wrap').hide();
		return;
