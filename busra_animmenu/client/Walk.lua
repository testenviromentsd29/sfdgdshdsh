...
function WalkMenuStart(name)
    if Config.PersistentWalk then SetResourceKvp("walkstyle2", name) end
    ...
    while LocalPlayer.state.walkstyle and LocalPlayer.state.walkstyle == name do
        Wait(1000);
        if GetPedMovementClipset(PlayerPedId()) ~= name and GetPedMovementClipset(PlayerPedId()) ~= `move_ped_crouched` then
            RequestWalking(name)
            SetPedMovementClipset(PlayerPedId(), name, 0.2)
            RemoveAnimSet(name)
        end
...
if Config.WalkingStyles and Config.PersistentWalk then
    AddEventHandler('playerSpawned', function()
        local kvp = GetResourceKvpString("walkstyle2")
