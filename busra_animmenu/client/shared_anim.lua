...
AddEventHandler("ClientEmoteRequestReceive", function(anim, etype)
    ...
    if not anim or not anim.sharedAnim then return end

    isRequestAnim = true
    requestedemote = anim.animName

    -- print("Requested to do ",requestedemote, anim.sharedAnim)
...
function EmoteInviteLoop()
    local startTime = GetGameTimer()
    while isRequestAnim do
        Wait(0)
        ESX.ShowHelpNotification("~INPUT_MP_TEXT_CHAT_TEAM~ Accept emote request\n~INPUT_CELLPHONE_CAMERA_FOCUS_LOCK~ Deny emote request")
        if IsControlJustPressed(1, 246) and isRequestAnim then
            ...
            if (distance ~= -1 and distance < 3) then
                ...
                print("accepted shared emote", requestedemote, otheremote)
        ...
        elseif IsControlJustPressed(1, 182) and isRequestAnim then
            ...
            target, distance = GetClosestPlayer()
            if (distance ~= -1 and distance < 3) then
                TriggerServerEvent("ServerDeclinedEmote", GetPlayerServerId(target))
            end
            isRequestAnim = false
        end
        if startTime + 10000 < GetGameTimer() then
            SimpleNotify(Config.Languages[lang]['refuseemote'])
            isRequestAnim = false
        end
    end
