...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('gunshop:getGunshops', function(gunshops)
		for k,v in ipairs(gunshops) do
			Config.Gunshops[v.job] = {
				coords = v.coords,
				heading = v.heading,
...
RegisterNetEvent('gunshop:addGunshop')
AddEventHandler('gunshop:addGunshop', function(gunshop)
	Config.Gunshops[gunshop.job] = {
		coords = gunshop.coords,
		heading =  gunshop.heading,
		model = `s_m_y_ammucity_01`,
		menu = 'default',
	}
end)

RegisterNetEvent('gunshop:buy')
AddEventHandler('gunshop:buy', function(item, gunshop)
...
RegisterNUICallback('buy', function(data, cb)
	if string.find(data.name, 'WEAPON_') then
		...
		if not isBusy then
			TriggerServerEvent('gunshop:buy', data.gunshop, data.type, data.id, data.count)
		end
	else
	...
	Citizen.CreateThread(function()
		while true do
			...
			for k,v in pairs(Config.Gunshops) do
				if #(coords - v.coords) < 50.0 and (currentBucket == 0 or (currentBucket > 0 and currentBucketName == k) or (currentBucket > 0 and string.find(k, currentBucketName))) then
